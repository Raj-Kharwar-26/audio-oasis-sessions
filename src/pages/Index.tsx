...
const Index = () => {
  ...
  const handleCopy = (text: string, what: string) => {
    navigator.clipboard.writeText(text).then(() => toast.success(`${what} copied to clipboard`)).catch(() => toast.error('Failed to copy to clipboard'));
  };
  if (isLoading) {
    return <Layout>
  ...
  return <Layout>
      <div className="space-y-8">
        ...
        {/* My Sessions Section */}
        {mySessions.length > 0 && <div className="space-y-4">
            <h2 className="text-2xl font-bold">My Sessions</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {mySessions.map(session => <Card key={session.id} className="glass-card overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-primary/10 hover:-translate-y-1">
                  <CardHeader>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      {session.playlist.length > 0 && session.playlist[session.currentSongIndex] && <div className="mt-4 p-2 bg-secondary/20 rounded-md">
                  ...
                  <div className="flex px-6 pb-6 gap-2">
                    <Button className="flex-1" onClick={() => joinSession(session.id)}>
                      Join
                    </Button>
          ...
          {selectedSession && <div className="space-y-4 py-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" className="h-8 px-2 text-xs" onClick={() => handleCopy(selectedSession.roomId, 'Room ID')}>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" className="h-8 px-2 text-xs" onClick={() => handleCopy(getSessionShareLink(selectedSession.id), 'Share link')}>
                    <Copy size={14} className="mr-1" />
                    Copy
                  </Button>
                </div>
